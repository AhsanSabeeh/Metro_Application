...
public:
    ...
    static void Create_Metro_Map(Graph_M &g)
    {
        ...
        unordered_set<string> stationSet;
        string line;
        getline(file, line);
        while(getline(file,line)) 
        {
            stringstream ss(line);
            string s1, s2, distance;
            getline(ss,s1,',');
            getline(ss,s2,',');
            getline(ss, distance, ',');
            stationSet.insert(s1);
            stationSet.insert(s2);
         
            routes.push_back({s1, s2, distance}); 
        }

        // file.close();
        //cout << "Distinct metro stations" << endl;
        for(const string& station: stationSet)
        {
            g.addVertex(station);
        }
...
//         }
        ...
        {
            g.addEdge(route[0], route[1], stoi(route[2]));
        }
        file.close();
...
// void add_New_Station(Graph_M &g)
// {
//     string station_name;
...
//     if (g.containsVertex(station_name)) {
//         cout << "Station already exists." << endl;
//         return;
//     }

//     // Add the new station
//     g.addVertex(station_name);
//     cout << "Station '" << station_name << "' added successfully." << endl;
//}
...
int main()
{
    ...
    while (true)
    {
        ...
        cout << "5. GET SHORTEST PATH WITH INTERCHANGE(DISTANCE WISE) TO REACH FROM A 'SOURCE' STATION TO 'DESTINATION' STATION" << endl;
        cout << "6. GET SHORTEST PATH WITH INTERCHANGE(TIME WISE) TO REACH FROM A 'SOURCE' STATION TO 'DESTINATION' STATION" << endl;
        ...
        {   g.display_Stations();
            ...
            else
            {
                ...
                //  cout << "DISTANCE : " << str[len - 3] << endl;
                //  cout << "NUMBER OF INTERCHANGES : " << str[len - 2] << endl;
                cout<< str[len-3] << "KMS   ==>    END";
                cout << "\n~~~~~~~~~~~~~" << endl;
            }
            break;
        }
